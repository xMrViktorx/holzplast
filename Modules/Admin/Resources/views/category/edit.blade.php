        ...
        <form method="POST" action="{{ route('admin.category.update', $category->id) }}">
            ...
            <div class="mb-6">
                <label for="status" class="inline-flex relative items-center mb-4 cursor-pointer">
                    <input type="checkbox" name="status" id="status" value="1" class="sr-only peer" {{ $category->status ? 'checked' : '' }}>
            ...
            <div class="mb-6">
                ...
                <select id="parent_category" name="parent_category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    ...
                    @foreach ($categories as $main_category)
                        <option value="{{ $main_category->id }}" {{ $main_category->id == $category->parent_category_id || old('parent_category_id') == $main_category->id ? 'selected' : '' }}>
