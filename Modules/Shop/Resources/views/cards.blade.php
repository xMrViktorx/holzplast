<div class="pt-32 flex flex-wrap justify-center gap-6">
    @if (count($products))
        @foreach ($products as $product)
            <div class="w-96 max-h-[42rem] rounded overflow-hidden shadow-lg flex flex-col mb-8">
                ...
                <div class="flex-1 px-6 py-4 flex flex-col justify-between">
                    <div>
                        ...
                        <div class="text-base line-clamp-4">
                            {!! $product->description !!}
                        </div>
                    </div>
                    <div class="flex flex-col">
                        <div class="font-bold text-2xl mb-2 mt-6">{{ formatPrice($product->price) }} + Áfa</div>
                        <a href="/{{ $product->category->slug }}/{{ $product->slug }}" class="bg-backgroundNavbar px-8 py-2 text-lg text-center cursor-pointer rounded shadow-lg hover:bg-backgroundMain">Megrendelés</a>
                    </div>
                </div>
            </div>
        @endforeach
